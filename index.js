...
async function run() {
  try {
    ...
    app.get('/recipes', async (req, res) => {
      ...
      const cursor = recipeCollection.find(query).sort({ recipe_name: 1 }).collation({ locale: "en", caseLevel: true });
    ...
    app.get('/ratings/:recipeId', async (req, res) => {
      try {
        ...
        const ratings = await ratingCollection.find({ recipeId }).toArray();
